...
def parse_email(path) -> dict:
    """Extract from/subject/date/body from an exported .eml or .txt file.

    Headers are the leading `Key: value` lines; the body is everything after the
    first blank line (falling back to any unparsed remainder).
    """
    path = Path(path)
...
def _taxonomy_block(config: dict) -> str:
    ...
    st = "\n".join(f"  - {s}: {sd.get(s, '')}".rstrip() for s in config["statuses"])
