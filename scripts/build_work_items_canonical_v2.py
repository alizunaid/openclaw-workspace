...
OUT_CSV = Path.home() / ".openclaw/workspace/WORK_ITEMS_REGISTER.canonical.v2.csv"
...
def normalize_text(s: str) -> str:
    s = (s or "").strip().lower()
    # remove repeated re/fw/fwd prefixes
    while True:
        new = RE_PREFIX.sub("", s).strip()
        if new == s:
            break
        s = new
    # normalize apostrophes and common typos/variants
    s = s.replace("’", "'")
...
def main():
    ...
    print(f"Wrote: {OUT_CSV} ({len(items)} canonical v2 work items)")
    print("Top 15 canonical v2 work items:")
    for it in items[:15]:
